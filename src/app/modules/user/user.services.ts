...
const createDoctor = async (req: Request) => {
  ...
  if (file) {
    ...
    req.body.data.doctor.profilePhoto = uploadedProfileImage?.secure_url;
  }

  const hashPassword = await hashedPassword(req.body.data.password);
  const result = await prisma.$transaction(async transactionClient => {
    await transactionClient.user.create({
      data: {
        email: req.body.data.doctor.email,
